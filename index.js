...
const BotName = 'Toin'; 
const youtube = 'https://www.youtube.com/channel/UCCnIn8MKmVfpNIKkeZwMU3A'; 
const aktif = '12:00 - 12:00';
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n' 
            + 'FN:Mrf.zvx\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5562996638900:+55 62 99664 - 8900\n' //Nomor whatsapp kamu
            + 'END:VCARD'
const
{
WAConnection,
...
if (text.includes(".neko"))
   ...
   {
    var items = ["anime trap"];
    var nime = items[Math.floor(Math.random() * items.length)];
    var url = "https://tobz-api.herokuapp.com/api/nsfwtrap" + nime;
    ...
    axios.get(url)
      .then((result) => {
        ...
        .then(
            (response) => {
    conn.sendMessage(id, '[ Espera Opoha ] Procurano ⏳ a Trap', MessageType.text, { quoted: m } )
	var buf = Buffer.from(response, 'base64'); 
              conn.sendMessage(id, buf ,MessageType.image, { caption: `👉👈 to duror`, quoted: m } )
            }
        )
        .catch(
            (error) => {
